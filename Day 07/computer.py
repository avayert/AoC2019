...
tape = [int(instruction) for instruction in sys.argv[1].split(',')]
...
handlers = {
    ...
    3: writing(functools.partial(input, '')),
    4: lambda value_at_index: print(value_at_index),
    5: lambda value, position: set_cursor(position) if value else None,
    6: lambda value, position: set_cursor(position) if not value else None,
    7: writing(operator.lt),
    8: writing(operator.eq),
